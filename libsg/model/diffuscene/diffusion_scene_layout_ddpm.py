...
class DiffusionSceneLayout_DDPM(Module):
    ...
    def __init__(self, n_classes: int, feature_extractor, config):
        ...
        if self.room_mask_condition:
            ...
            self.fc_room_f = nn.Linear(self.feature_extractor.feature_size, config["latent_dim"])
    ...
    def get_loss(self, sample_params):
        ...
        if self.instance_condition:
            if self.learnable_embedding:
                instance_indices = (
                    torch.arange(self.sample_num_points).long().to(device)[None, :].repeat(batch_size, 1)
                )
                instan_condition_f = self.positional_embedding[instance_indices, :]
            else:
                instance_label = (
                    torch.eye(self.sample_num_points).float().to(device)[None, ...].repeat(batch_size, 1, 1)
                )
        ...
        if self.room_arrange_condition:
            arrange_input = torch.cat(
                [
                    room_layout_target[:, :, self.translation_dim : self.translation_dim + self.size_dim],
            ...
            room_layout_target = torch.cat(
                [
                    room_layout_target[:, :, 0 : self.translation_dim],
                    room_layout_target[:, :, self.translation_dim + self.size_dim : self.bbox_dim],
                ],
                dim=-1,
            ).contiguous()
        ...
        loss, loss_dict = self.diffusion.get_loss_iter(
            room_layout_target, condition=condition, condition_cross=condition_cross
        )
    ...
    def sample(
        ...
        if self.instance_condition:
            if self.learnable_embedding:
                instance_indices = (
                    torch.arange(self.sample_num_points).long().to(device)[None, :].repeat(room_mask.size(0), 1)
                )
                instan_condition_f = self.positional_embedding[instance_indices, :]
            else:
                instance_label = (
                    torch.eye(self.sample_num_points).float().to(device)[None, ...].repeat(room_mask.size(0), 1, 1)
                )
                instan_condition_f = self.fc_instance_condition(instance_label)
        else:
            instan_condition_f = None
        ...
        if self.room_arrange_condition:
            arrange_input = torch.cat(
                [
                    input_boxes[:, :, self.translation_dim : self.translation_dim + self.size_dim],
        ...
        # reverse sampling
        if ret_traj:
            samples = self.diffusion.gen_sample_traj(
                noise.shape,
                room_mask.device,
                freq=freq,
                condition=condition,
            ...
            samples = self.diffusion.gen_samples(
                noise.shape,
                room_mask.device,
                condition=condition,
                condition_cross=condition_cross,
    ...
    @torch.no_grad()
    def generate_layout_progressive(
        self,
        room_mask,
        num_points,
        point_dim,
    ...
    def delete_empty_from_network_samples(self, samples, device="cpu", keep_empty=False):
        ...
        if self.objfeat_dim > 0:
            samples_dict["objfeats"] = samples[
                :, :, self.bbox_dim + self.class_dim : self.bbox_dim + self.class_dim + self.objfeat_dim
            ]
                ...
                for k in samples_dict.keys():
                    if k == "class_labels":
                        ...
                        boxes[k] = torch.cat(
                            [
                                boxes[k],
                                samples[:, i : i + 1, self.bbox_dim : self.bbox_dim + self.class_dim - 1].to(device),
                            ],
                            dim=1,
                        )
                        boxes["objectness"] = torch.cat(
                            [
                                boxes["objectness"],
                                samples[
                                    :, i : i + 1, self.bbox_dim + self.class_dim - 1 : self.bbox_dim + self.class_dim
                                ].to(device),
                            ],
                            dim=1,
                        )
            ...
            if not keep_empty and samples_dict["class_labels"][0, i, -1] > 0:
                ...
                for k in samples_dict.keys():
                    if k == "class_labels":
                        # we output raw probability maps for visualization
                        boxes[k] = torch.cat(
                            [boxes[k], samples_dict[k][:, i : i + 1, : self.class_dim - 1].to(device)], dim=1
                        )
                        boxes["objectness"] = torch.cat(
                            [boxes["objectness"], samples_dict[k][:, i : i + 1, -1:].to(device)], dim=1
                        )
                    else:
                        boxes[k] = torch.cat([boxes[k], samples_dict[k][:, i : i + 1, :].to(device)], dim=1)
