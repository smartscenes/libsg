...
class GaussianDiffusion:
    def __init__(
        self, config, betas, loss_type, model_mean_type, model_var_type, loss_separate, loss_iou, train_stats_file
    ):
        ...
        if self.loss_iou:
            ...
            self._centroids_min, self._centroids_max = (
                torch.from_numpy(self._centroids[0]).float(),
                torch.from_numpy(self._centroids[1]).float(),
            )
            print(
                "load centroids min {} and max {} in Gaussian Diffusion".format(self._centroids[0], self._centroids[1])
            )

            self._sizes = train_stats["bounds_sizes"]
            self._sizes = (np.array(self._sizes[:3]), np.array(self._sizes[3:]))
            self._sizes_min, self._sizes_max = (
                torch.from_numpy(self._sizes[0]).float(),
                torch.from_numpy(self._sizes[1]).float(),
            )
            print("load sizes min {} and max {} in Gaussian Diffusion".format(self._sizes[0], self._sizes[1]))
    ...
    def p_mean_variance(
        self, denoise_fn, data, t, condition, condition_cross, clip_denoised: bool, return_pred_xstart: bool
    ):
    ...
    def _predict_eps_from_start(self, x_t, t, x0):
        return (self._extract(self.sqrt_recip_alphas_cumprod.to(x_t.device), t, x_t.shape) * x_t - x0) / self._extract(
    ...
    def p_sample(
        self, denoise_fn, data, t, condition, condition_cross, noise_fn, clip_denoised=False, return_pred_xstart=False
    ):
        """
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, condition=None, condition_cross=None):
        ...
        if self.loss_type == "mse":
            ...
            if data_start.shape[-1] == self.objectness_dim + self.class_dim + self.bbox_dim + self.objfeat_dim:
                loss_trans = (
                    (target[:, :, 0 : self.translation_dim] - denoise_out[:, :, 0 : self.translation_dim]) ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                loss_size = (
                    (
                        target[:, :, self.translation_dim : self.translation_dim + self.size_dim]
                        - denoise_out[:, :, self.translation_dim : self.translation_dim + self.size_dim]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                loss_angle = (
                    (
                        target[:, :, self.translation_dim + self.size_dim : self.bbox_dim]
                        - denoise_out[:, :, self.translation_dim + self.size_dim : self.bbox_dim]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                ...
                loss_class = (
                    (
                        target[:, :, self.bbox_dim : self.bbox_dim + self.class_dim]
                        - denoise_out[:, :, self.bbox_dim : self.bbox_dim + self.class_dim]
                    )
                    ** 2
                ).mean(dim=list(range(1, len(data_start.shape))))
                if self.objectness_dim == 0:
                    loss_object = (
                        (
                            target[:, :, self.bbox_dim + self.class_dim - 1 : self.bbox_dim + self.class_dim]
                            - denoise_out[:, :, self.bbox_dim + self.class_dim - 1 : self.bbox_dim + self.class_dim]
                        )
                        ** 2
                    ).mean(dim=list(range(1, len(data_start.shape))))
                else:
                    loss_object = (
                        (
                            target[
                                :,
                                :,
                                self.bbox_dim + self.class_dim : self.bbox_dim + self.class_dim + self.objectness_dim,
                            ]
                            - denoise_out[
                                :,
                                :,
                ...
                if self.objfeat_dim == 0:
                    ...
                    loss_objfeat = (
                        (
                            target[:, :, self.bbox_dim + self.class_dim + self.objectness_dim :]
                            - denoise_out[:, :, self.bbox_dim + self.class_dim + self.objectness_dim :]
                        )
                        ** 2
                    ).mean(dim=list(range(1, len(data_start.shape))))
                ...
                if self.loss_iou:
                    ...
                    descale_trans = self.descale_to_origin(
                        trans_recon,
                        self._centroids_min.to(data_start.device),
                        self._centroids_max.to(data_start.device),
                    )
                    ...
                    axis_aligned_bbox_corn = torch.cat(
                        [descale_trans - descale_sizes, descale_trans + descale_sizes], dim=-1
                    )
                    ...
                    loss_iou_valid_avg = (w_iou * 0.5 * bbox_iou_valid).sum(
...
class DiffusionPoint(nn.Module):
    def __init__(
        ...
        self.diffusion = GaussianDiffusion(
            config, betas, loss_type, model_mean_type, model_var_type, loss_separate, loss_iou, train_stats_file
        )
    ...
    def all_kl(self, x0, condition, condition_cross, clip_denoised=True):
        total_bpd_b, vals_bt, prior_bpd_b, mse_bt = self.diffusion.calc_bpd_loop(
            self._denoise, x0, condition, condition_cross, clip_denoised
        )
