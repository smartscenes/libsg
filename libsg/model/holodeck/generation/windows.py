...
class WindowGenerator:
    ...
    def generate_windows(self, scene, additional_requirements_window):
        ...
        for row in rows:
            ...
            # get wall id
            try:
                wall_id = organized_walls[room_id][parsed_plan["wall_direction"]]["wall_id"]
            except:
    ...
    def select_window(self, window_type, window_size):
        candidate_window_ids = [
            window_id for window_id in self.window_ids if self.window_data[window_id]["type"] == window_type
        ]
        size_differences = [
            np.linalg.norm(np.array(window_size) - np.array(self.window_data[window_id]["size"]))
