...
class ObjathorRetriever:
    def __init__(
        ...
        assert objathor_clip_features_dict["uids"] == objathor_sbert_features_dict["uids"]
        ...
        # thor_sbert_features = thor_sbert_features_dict["text_features"].astype(np.float32)

        # self.clip_features = torch.from_numpy(np.concatenate([objathor_clip_features, thor_clip_features], axis=0))
        self.clip_features = torch.from_numpy(np.concatenate([objathor_clip_features], axis=0))
        self.clip_features = F.normalize(self.clip_features, p=2, dim=-1)

        # self.sbert_features = torch.from_numpy(np.concatenate([objathor_sbert_features, thor_sbert_features], axis=0))
        self.sbert_features = torch.from_numpy(np.concatenate([objathor_sbert_features], axis=0))
