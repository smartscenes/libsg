...
from libsg.model.holodeck.constants import HOLODECK_BASE_DATA_DIR
...
class DoorGenerator:
        ...
        try:
            ...
            for door_id in tqdm(self.door_ids):
                image = self.preprocess(
                    Image.open(os.path.join(HOLODECK_BASE_DATA_DIR, f"doors/images/{door_id}.png"))
                ).unsqueeze(0)
                with torch.no_grad():
    ...
    def generate_doors(self, scene, additional_requirements_door):
        ...
        for i, plan in enumerate(plans):
            ...
            # get door asset
            if exterior:
                parsed_plan["connection_type"] = "doorway"  # force to use doorway for exterior
        ...
        if len(unconnected_rooms) > 0:
            for room in unconnected_rooms:
                ...
                current_walls = [
                    wall
                    for wall in walls
                    if wall["roomId"] == room and "exterior" not in wall["id"] and len(wall["connected_rooms"]) != 0
                ]
    ...
    def get_connection(self, room0_id, room1_id, walls):
        ...
        if len(valid_connections) == 0:
            print(f"{Fore.RED}There is no wall between {room0_id} and {room1_id}{Fore.RESET}")
            return None
    ...
    def get_connection_exterior(self, room0_id, room1_id, walls):
        ...
        interior_walls = [wall["id"] for wall in walls if wall["roomId"] == room_id and "exterior" not in wall["id"]]
        exterior_walls = [wall["id"] for wall in walls if wall["roomId"] == room_id and "exterior" in wall["id"]]
        wall_pairs = []
    ...
    def get_room_pairs_str(self, rooms, walls):
        ...
        for pair in room_pairs:
            if pair not in room_pairs_no_dup and (pair[1], pair[0]) not in room_pairs_no_dup:
    ...
    def get_random_door(self, wall_width):
        single_doors = [door_id for door_id in self.door_ids if self.door_data[door_id]["size"] == "single"]
        double_doors = [door_id for door_id in self.door_ids if self.door_data[door_id]["size"] == "double"]
