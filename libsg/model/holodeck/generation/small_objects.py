...
from libsg.model.holodeck.constants import THOR_COMMIT_ID
from libsg.model.holodeck.generation.objaverse_retriever import ObjathorRetriever
from libsg.model.holodeck.generation.utils import (
    get_bbox_dims,
    get_annotations,
...
class ControllerProcess(multiprocessing.Process):
    def __init__(self, receive_queue, send_queue, objaverse_dir, scene):
        ...
        if self._controller is None:
            self._controller = Controller(
                ...
                action_hook_runner=ProceduralAssetHookRunner(
                    asset_directory=self.objaverse_dir,
                    asset_symlink=True,
                    verbose=True,
                ),
            )
    ...
    def place_object(self, object_id, receptacle_id):
    ...
    def run(self):
        ...
        while True:
            ...
            object_id, receptacle_id = message

            obj = self.place_object(object_id, receptacle_id)
...
class SmallObjectGenerator:
        ...
        for receptacle, small_objects in receptacle2small_objects.items():
            ...
            for object_name, asset_id, _ in small_objects:
                ...
                if obj != None:  # If the object is successfully placed
                    ...
                    placement["dimension"] = dimensions
                    asset_height = dimensions["y"]
    ...
    def select_small_objects_per_receptacle(self, args):
        ...
        for small_object in small_objects:
            ...
            if len(valid_candidates) > 1:
                valid_candidates = [candidate for candidate in valid_candidates if candidate[0] not in self.used_assets]
            if len(valid_candidates) == 0:
    ...
    def place_object(self, controller, object_id, receptacle_id, rotation=[0, 0, 0]):
        if isinstance(controller, ControllerProcess):
            controller.receive_queue.put((object_id, receptacle_id))
            return controller.send_queue.get(block=True)
        else:
            generated_id = f"small|{object_id}"
            # Spawn the object
