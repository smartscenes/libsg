...
from libsg.model.holodeck.constants import HOLODECK_BASE_DATA_DIR, DEBUGGING
...
class MaterialSelector:
    ...
    def load_features(self):
        try:
            self.material_feature_clip = compress_pickle.load(
                os.path.join(HOLODECK_BASE_DATA_DIR, "materials/material_feature_clip.pkl")
            )
        except:
            ...
            for material in tqdm(self.selected_materials):
                image = self.preprocess(
                    Image.open(os.path.join(HOLODECK_BASE_DATA_DIR, f"materials/images/{material}.png"))
                ).unsqueeze(0)
                with torch.no_grad():
    ...
    def match_material(self, queries, topk=5):
        ...
        string_similarity = torch.tensor(
            [[self.string_match(query, material) for material in self.selected_materials] for query in queries]
        )
