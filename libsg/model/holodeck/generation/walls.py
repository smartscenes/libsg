...
class WallGenerator:
    def __init__(self, llm: OpenAI):
        ...
        self.llm = llm
        self.wall_height_template = PromptTemplate(input_variables=["input"], template=prompts.wall_height_prompt)
        self.used_assets = []
    ...
    def generate_walls(self, scene):
        ...
        for room in rooms:
            ...
            for j in range(len(full_vertices)):
                ...
                wall["polygon"] = self.generate_wall_polygon(wall_endpoint1, wall_endpoint2, wall_height)

                # add the room connection information
                wall["connected_rooms"] = self.get_connected_rooms(wall["polygon"], rooms, wall["roomId"])

                # add the wall direction and width
                wall_width, wall_direction = self.get_wall_direction(wall_endpoint1, wall_endpoint2, full_vertices)
        ...
        for wall in walls:
            if wall["connected_rooms"] != []:
                for connection in wall["connected_rooms"]:
                    ...
                    candidate_walls = [wall for wall in walls if wall["roomId"] == connect_room_id]
                    line1 = connection["line1"]
                    for candidate_wall in candidate_walls:
                        if line1[0] in candidate_wall["polygon"] and line1[1] in candidate_wall["polygon"]:
    ...
    def get_wall_height(self, scene):
        ...
        if "wall_height" not in scene:
            ...
            try:
                ...
                wall_height = round(random.uniform(2.5, 4.5), 1)  # if failed, random height between 2.5 and 4.5

            scene["wall_height"] = min(max(wall_height, 2.0), 4.5)  # limit the wall height between 2.0 and 4.5
    ...
    def get_connected_rooms(self, wall_polygon, rooms, roomId):
        connected_rooms = []
        vertices0 = [(vertex["x"], vertex["z"]) for vertex in wall_polygon if vertex["y"] == 0]
        ...
        for room in rooms:
            ...
            lines1 = [LineString([vertices1[i], vertices1[(i + 1) % len(vertices1)]]) for i in range(len(vertices1))]
    ...
    def update_walls(self, original_walls, open_room_pairs):
        ...
        for segment in open_wall_segments:
            if segment not in open_wall_segments_no_overlap and segment[::-1] not in open_wall_segments_no_overlap:
    ...
    def create_rectangles(self, segment):
        ...
        perp_vec *= 0.5  # 0.5 is the hyperparameter for the width of the open connection
