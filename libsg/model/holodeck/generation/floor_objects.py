...
import libsg.model.holodeck.generation.prompts as prompts
from libsg.model.holodeck.generation.milp_utils import *
from libsg.model.holodeck.generation.objaverse_retriever import ObjathorRetriever
from libsg.model.holodeck.generation.utils import get_bbox_dims
...
class FloorObjectGenerator:
    ...
    def generate_objects(self, scene, use_constraint=True):
        ...
        packed_args = [(room, doors, windows, open_walls, selected_objects, use_constraint) for room in rooms]
    ...
    def solution2placement(self, solutions, object_name2id, room_id):
        ...
        for object_name, solution in solutions.items():
            if "door" in object_name or "window" in object_name or "open" in object_name:
                continue
            dimension = get_bbox_dims(self.database[object_name2id[object_name]])
            placement = self.json_template.copy()
            ...
            for constraint in constraints:
                ...
                try:
                    ...
                    _, new_constraint_name = min(
                        [(editdistance.eval(cn, constraint_name), cn) for cn in constraint_name2type]
                    )
                    print(f"constraint type {constraint_name} not found, using {new_constraint_name} instead.")
                    constraint_name = new_constraint_name
                    constraint_type = constraint_name2type[constraint_name]

                if constraint_type == "global":
                    object2constraints[object_name].append({"type": constraint_type, "constraint": constraint_name})
                    ...
                    if target in object2constraints:
                        ...
                        print(f"target object {target} not found in the existing constraint plan")
                        continue
                else:
    ...
    def order_objects_by_size(self, selected_floor_objects):
        ...
        ordered_floor_objects_no_size = [
            [object_name, asset_id] for object_name, asset_id, size in ordered_floor_objects
        ]
...
class DFS_Solver_Floor:
                ...
                for constraint in object_constraints:
                    if constraint["type"] == "distance":
                        ...
                        if target_object_name in constraints.keys():
                            ...
                            if any(
                                constraint["type"] == "distance" and constraint["target"] == object_name
    ...
    def get_possible_placements(self, room_poly, object_dim, constraints, grid_points, placed_objects):
        ...
        global_constraint = next(
            (constraint for constraint in constraints if constraint["type"] == "global"),
            None,
        )
        ...
        if global_constraint["constraint"] == "edge":
            ...
            if len(constraints) > 1:
                candidate_solutions = solutions + edge_solutions  # edge is soft constraint
            else:
    ...
    def place_relative(self, place_type, target_object, solutions):
        ...
        comparison_dict = {
            ...
            "in front of": {
                ...
                and mean_x - self.grid_size < sol_center[0] < mean_x + self.grid_size,  # in front of and centered
    ...
    def milp_dfs(self, room_poly, all_objects_list, constraints, placed_objects, branch_factor=1):
        ...
        def parse_object_properties(object_properties):
            ...
            object_dim = (
                (max_x - min_x, max_y - min_y) if rotation == 0 or rotation == 180 else (max_y - min_y, max_x - min_x)
            )
        ...
        for branch_idx in range(branch_factor):
            ...
            for object_name, object_dim in objects_list:
                ...
                all_other_objects_list = [x[0] for x in objects_list if x[0] != object_name] + list(
                    placed_objects.keys()
                )
