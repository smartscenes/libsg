...
class SgObjfeatVQDiffusion(nn.Module):
    ...
    def compute_losses(
        ...
        if self.training and self.cfg_drop_ratio > 0.0:
            empty_last_hidden_state = torch.zeros_like(text_last_hidden_state[0]).unsqueeze(
                0
            )  # e.g., for CLIP ViT-B/32: (1, 77, 512)
        ...
        _, _, _, vb_loss_x, vb_loss_e, vb_loss_o = self._train_loss(
            x, e, o, text_last_hidden_state, text_embeds, edge_weight
        )
    ...
    def generate_samples(
        ...
        if start_step == 0:
            ...
            zero_logits_e = torch.zeros(
                (batch_size, self.num_edge_classes - 1, num_nodes * (num_nodes - 1) // 2), device=device
            )
            ...
            zero_logits_o = torch.zeros(
                (batch_size, self.num_objfeat_classes - 1, num_nodes * 4), device=device
    ...
    @torch.no_grad()
    def complete(
        self,  # Condition on partial graphs, generate the rest
        sample_params: Dict[str, Tensor],
        ...
        if start_step == 0:
            ...
            for diffusion_index in tqdm(range(start_step - 1, -1, -1), desc="Completing graphs", ncols=125):
                t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                log_z_x_t = self.q_sample(
                    log_z_x_start, t, self.num_node_classes
                )  # mask the original `x` depending on `t`
                log_z_x = torch.where(x_mask.unsqueeze(1).bool(), log_z_x_mask, log_z_x_t)
                log_z_e_t = self.q_sample(
                    log_z_e_start, t, self.num_edge_classes
                )  # mask the original `e` depending on `t`
                log_z_e = torch.where(e_mask.unsqueeze(1).bool(), log_z_e_mask, log_z_e_t)
                log_z_o_t = self.q_sample(
                    log_z_o_start, t, self.num_objfeat_classes
                )  # mask the original `o` depending on `t`
    ...
    @torch.no_grad()
    def rearrange(
        self,  # Condition on `x` and `o`, generate `e`
        sample_params: Dict[str, Tensor],
        text_last_hidden_state: Optional[Tensor] = None,
        text_embeds: Optional[Tensor] = None,
        filter_ratio=0.5,
        ...
        if start_step == 0:
            ...
            for diffusion_index in tqdm(range(start_step - 1, -1, -1), desc="Rearranging graphs", ncols=125):
                t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                log_z_x = self.q_sample(
                    log_z_x_start, t, self.num_node_classes
                )  # mask the original `x` depending on `t`
                log_z_o = self.q_sample(
                    log_z_o_start, t, self.num_objfeat_classes
                )  # mask the original `o` depending on `t`
                log_z_x, log_z_e, log_z_o = self.p_sample(
                    log_z_x, log_z_e, log_z_o, t, text_last_hidden_state, text_embeds, cfg_scale, truncation_rate
    ...
    @torch.no_grad()
    def stylize(
        self,  # Condition on `x` and `e`, generate `o`
        sample_params: Dict[str, Tensor],
        text_last_hidden_state: Optional[Tensor] = None,
        text_embeds: Optional[Tensor] = None,
        filter_ratio=0.5,
    ...
    def q_pred_one_timestep(self, log_x_t: Tensor, t: LongTensor, num_classes: int):  # q(xt|xt_1)
        ...
        log_bt = extract(
            (self.log_bt.exp() * (self.num_node_classes - 1) / (num_classes - 1)).log(), t, log_x_t.shape
        )  # bt; -1 for [mask] token
    ...
    def q_pred(self, log_x_start: Tensor, t: LongTensor, num_classes: int):  # q(xt|x0)
        ...
        log_cumprod_bt = extract(
            (self.log_cumprod_bt.exp() * (self.num_node_classes - 1) / (num_classes - 1)).log(), t, log_x_start.shape
        )  # bt~; -1 for [mask] token
    ...
    def predict_start(
        ...
        t: LongTensor,
        cond_emb: Tensor,
        global_cond_emb: Optional[Tensor] = None,
        cfg_scale=1.0,
        truncation_rate=1.0,
    ):  # p(x0|xt)
        ...
        if self.zero_vector_x is None or self.zero_vector_x.shape[0] != batch_size:
            self.zero_vector_x = torch.zeros(batch_size, 1, *out_x.shape[2:]).type_as(log_x_t) - 70.0
        if self.zero_vector_e is None or self.zero_vector_e.shape[0] != batch_size:
            self.zero_vector_e = torch.zeros(batch_size, 1, *out_e.shape[2:]).type_as(log_e_t) - 70.0
        if self.zero_vector_o is None or self.zero_vector_o.shape[0] != batch_size:
            self.zero_vector_o = torch.zeros(batch_size, 1, *out_o.shape[2:]).type_as(log_o_t) - 70.0
        ...
        log_pred_x = torch.clamp(log_pred_x, -70.0, 0.0)
        log_pred_e = torch.clamp(log_pred_e, -70.0, 0.0)
        log_pred_o = torch.clamp(log_pred_o, -70.0, 0.0)

        return log_pred_x, log_pred_e, log_pred_o

    def q_posterior(
        self, log_x_start: Tensor, log_x_t: Tensor, t: LongTensor, num_classes: int
        ...
        return torch.clamp(log_EV_xtmin_given_xt_given_xstart, -70.0, 0.0)
    ...
    def p_pred(
        ...
        t: LongTensor,
        cond_emb: Tensor,
        global_cond_emb: Optional[Tensor],
        cfg_scale=1.0,
        truncation_rate=1.0,
    ):  # if x0, first p(x0|xt), than sum( q(xt-1|xt,x0) * p(x0|xt) )
        if self.parametrization == "x0":
            log_x_recon, log_e_recon, log_o_recon = self.predict_start(
                log_x, log_e, log_o, t, cond_emb, global_cond_emb, cfg_scale, truncation_rate
            )
            log_model_pred_x = self.q_posterior(
                log_x_start=log_x_recon, log_x_t=log_x, t=t, num_classes=self.num_node_classes
            )
            log_model_pred_e = self.q_posterior(
                log_x_start=log_e_recon, log_x_t=log_e, t=t, num_classes=self.num_edge_classes
            )
            log_model_pred_o = self.q_posterior(
                log_x_start=log_o_recon, log_x_t=log_o, t=t, num_classes=self.num_objfeat_classes
            )
    ...
    def p_sample(
        ...
        log_model_pred_x, log_model_pred_e, log_model_pred_o, _, _, _ = self.p_pred(
            log_x, log_e, log_o, t, cond_emb, global_cond, cfg_scale, truncation_rate
        )
    ...
    def log_sample_categorical(
        self, logits: Tensor, num_classes: int
    ...
    def _train_loss(
        ...
        log_x0_recon, log_e0_recon, log_o0_recon = self.predict_start(
            log_xt, log_et, log_ot, t, cond_emb, global_cond_emb
        )  # P_theta(x0|xt)
        log_model_prob_x = self.q_posterior(
            log_x_start=log_x0_recon, log_x_t=log_xt, t=t, num_classes=self.num_node_classes
        )  # go through q(xt_1|xt,x0)
        log_model_prob_e = self.q_posterior(
            log_x_start=log_e0_recon, log_x_t=log_et, t=t, num_classes=self.num_edge_classes
        )  # go through q(xt_1|xt,x0)
        log_model_prob_o = self.q_posterior(
            log_x_start=log_o0_recon, log_x_t=log_ot, t=t, num_classes=self.num_objfeat_classes
        )  # go through q(xt_1|xt,x0)

        # Compute log_true_prob now
        log_true_prob_x = self.q_posterior(
            log_x_start=log_x_start, log_x_t=log_xt, t=t, num_classes=self.num_node_classes
        )
        log_true_prob_e = self.q_posterior(
            log_x_start=log_e_start, log_x_t=log_et, t=t, num_classes=self.num_edge_classes
        )
        log_true_prob_o = self.q_posterior(
            log_x_start=log_o_start, log_x_t=log_ot, t=t, num_classes=self.num_objfeat_classes
        )

        # Compute loss
        kl_x = self.multinomial_kl(log_true_prob_x, log_model_prob_x)  # (B, N)
        mask_region_x = (xt == self.num_node_classes - 1).float()
        mask_weight_x = mask_region_x * self.mask_weight[0] + (1.0 - mask_region_x) * self.mask_weight[1]
        ...
        mask_weight_e = mask_region_e * self.mask_weight[0] + (1.0 - mask_region_e) * self.mask_weight[1]
        ...
        mask_weight_o = mask_region_o * self.mask_weight[0] + (1.0 - mask_region_o) * self.mask_weight[1]
        ...
        kl_loss_x = mask * decoder_nll_x + (1.0 - mask) * kl_x  # (B,)
        kl_loss_e = mask * decoder_nll_e + (1.0 - mask) * kl_e  # (B,)
        kl_loss_o = mask * decoder_nll_o + (1.0 - mask) * kl_o  # (B,)
        ...
        if self.auxiliary_loss_weight != 0.0:
            ...
            kl_aux_loss_x = mask * decoder_nll_x + (1.0 - mask) * kl_aux_x
            ...
            kl_aux_loss_e = mask * decoder_nll_e + (1.0 - mask) * kl_aux_e
            ...
            kl_aux_loss_o = mask * decoder_nll_o + (1.0 - mask) * kl_aux_o

            if self.adaptive_auxiliary_loss:
                addition_loss_weight = (1.0 - t / self.num_timesteps) + 1.0
            else:
    ...
    def truncate(self, log_p_x_0: Tensor, truncation_rate=1.0):
...
def log_1_min_a(a: Tensor):
    return torch.log(1.0 - a.exp() + 1e-40)
...
class SgObjfeatTransformerVQDiffusionWrapper(nn.Module):
    def __init__(
        ...
        self.node_embed = nn.Sequential(nn.Embedding(node_dim, attn_dim), nn.GELU(), nn.Linear(attn_dim, attn_dim))

        self.edge_embed = nn.Sequential(
            nn.Embedding(edge_dim, attn_dim // 4), nn.GELU(), nn.Linear(attn_dim // 4, attn_dim // 4)
        )
        ...
        self.objfeat_pool_transformer = nn.ModuleList(
            [
                BasicTransformerBlock(  # self-attn + ff
                    objfeat_dim, objfeat_dim, None, None, n_heads, gated_ff, dropout
                )
        ...
        self.objfeat_pool = nn.Sequential(
            nn.Linear(objfeat_dim, objfeat_dim), nn.GELU(), nn.Linear(objfeat_dim, attn_dim)
        )

        if global_condition_dim is not None:
            if concat_global_condition:
                self.global_condition_embed = nn.Sequential(
                    nn.Linear(global_condition_dim, context_dim), nn.GELU(), nn.Linear(context_dim, context_dim)
                )
            else:
                self.global_condition_embed = nn.Sequential(
        ...
        self.node_proj_out = nn.Sequential(
            nn.LayerNorm(attn_dim), nn.Linear(attn_dim, node_dim - 1)  # -1 for the [mask] token
        )
        self.edge_proj_out = nn.Sequential(
            nn.LayerNorm(attn_dim // 4), nn.Linear(attn_dim // 4, edge_dim - 1)  # -1 for the [mask] token
        )

        self.out_objfeat_tokens = nn.Parameter(torch.empty(4, attn_dim))  # TODO: make `4` configurable
        self.out_objfeat_tokens.data.uniform_(-1.0 / attn_dim, 1.0 / attn_dim)
        self.out_objfeat_transformer = nn.ModuleList(
            [
                BasicTransformerBlock(  # self-attn + cross-attn + ff
                    attn_dim, attn_dim, attn_dim, None, n_heads, gated_ff, dropout
                )
                for _ in range(2)  # TODO: make it configurable
            ]
        )
        ...
        self.objfeat_out = nn.Sequential(
            nn.Linear(attn_dim, attn_dim), nn.GELU(), nn.Linear(attn_dim, num_objfeat_embeds + 1)  # +1 for empty token
        )
    ...
    def forward(
        ...
        assert torch.all(
            e_onehot.sum(dim=-1) == (~eye_mask).float()
        )  # every edge is one-hot encoded, except for the diagonal
        ...
        if global_condition is not None:
            if self.concat_global_condition:
                condition = torch.cat([condition, self.global_condition_embed(global_condition).unsqueeze(1)], dim=1)
            else:
        ...
        o_emb = o_emb + get_1d_sincos_encode(
            torch.arange(o_emb.shape[1], device=o_emb.device), o_emb.shape[-1], o_emb.shape[1]
        ).unsqueeze(0)
        ...
        if self.use_positional_encoding:
            ...
            inst_emb_e = get_1d_sincos_encode(
                torch.arange(e_emb.shape[1], device=e_emb.device), e_emb.shape[-1], e_emb.shape[1]
            ).unsqueeze(
                0
            )  # (1, N, De)
        ...
        out_e = (out_e + out_e_negative.permute(0, 2, 1, 3)) / 2.0
