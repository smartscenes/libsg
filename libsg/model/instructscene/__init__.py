...
def model_from_config(config: Dict[str, Any], num_objs=-1, num_preds=-1, text_emb_dim=512, **kwargs):
    # Scene graph to scene layout
    if "sg2sc" in config["name"]:
        if "diffusion" in config["name"]:
            return Sg2ScDiffusion(num_objs, num_preds, use_objfeat="objfeat" in config["name"])
        else:
    ...
    # Instruction to scene graph
        ...
        if "vq_objfeat" in config["name"]:
            return SgObjfeatVQDiffusion(num_objs, num_preds, text_emb_dim=text_emb_dim)
        else:
            raise NotImplementedError(f"Unknown model name: {config['name']}")

    # Object feature VQ VAE
    elif "objfeatvqvae" in config["name"]:
        return ObjectFeatureVQVAE(config["objfeat_type"], config["vq_type"], **kwargs)
